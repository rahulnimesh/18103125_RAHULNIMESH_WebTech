...
public class a41 {
    ...
    public static int recurutil(String[] crops, int n, int m, int x, int y) {
        for(int i=x;i<n;i++) {
            for(int j=y;j<m;j++) {
                ...
                if((C == D && C!=(int)'#') || (C == R && C!=(int)'#') || (C == L&& C!=(int)'#') || (C == U && C!=(int)'#')) {
                    ...
                    int ans1 = 0;
                    if(j+1<m) ans1 = recurutil(crops, n, m, i, j+1)+1;
                    else ans1 = recurutil(crops, n, m, i+1, 0)+1;

                    crops[i] = saved;
                    int ans2 = 0;
                    if(j+1<m) ans2 = recurutil(crops, n, m, i, j + 1);
                    else ans2 = recurutil(crops, n, m, i+1, 0);

                    return Math.min(ans1, ans2);
                }
                else {
                    if(j+1<m)
                        return recurutil(crops, n, m, i, j+1);
                    else
                        return recurutil(crops, n, m, i+1, 0);
                }
            }
        }
    ...
    public static int replant(String[] crops, int n, int m) {
        return recurutil(crops, n, m, 0, 0);
    }
